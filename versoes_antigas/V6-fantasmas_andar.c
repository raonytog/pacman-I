...
void RealizaJogo(tJogo jogo) {
    ...
    char aux;
    tFantasmas fants;
    tEstatisticas RetornaStats(jogo);

    while (scanf("%c%*c", &aux) == 1 && 
           jogo.stats.pacmanComeu != jogo.stats.qtdComidaJogo) {
        jogo = EfetuaJogadaPacman(jogo, aux, jogo.stats); // jogada do pacman
...
tJogo EfetuaJogadaFantasmas (tJogo jogo) {
    ...
    tCoordenada coord;
    f = IdentificaFantasmasPresentes(jogo);
    if (fb.presenteNoJogo == 1) {
        fb.coordFantB = IdentificaCoordenadaFantasma(jogo, GHOST1);
        // if ()
    }
}
