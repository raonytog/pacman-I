...
void RetornaGameOver (tJogo jogo, char * diretorio);
void RetornaVitoria (tJogo jogo, char * diretorio);
...
void SaidaArquivoSaida(tJogo jogo, char* diretorio, char acao);
...
void RetornaGameOver (tJogo jogo, char * diretorio) {
    char dirSaida[DIR_TAM_MAX];
    sprintf(dirSaida, "%s/saida/resumo.txt", diretorio);

    FILE * tSaida = NULL;
    tSaida = fopen(dirSaida, "a+");

    if (!tSaida) {
        printf("Falha ao abrir o arquivo %s no gameover", dirSaida);
        exit(1);
    }

    fprintf(tSaida, "GAME OVER!\n");
    fprintf(tSaida, "Pontuação final: %d\n", RetornaPontuacao(jogo));
    fclose(tSaida);
}

void RetornaVitoria(tJogo jogo, char * diretorio) {
    char dirSaida[DIR_TAM_MAX];
    sprintf(dirSaida, "%s/saida/saida.txt", diretorio);
...
void SaidaArquivoSaida(tJogo jogo, char* diretorio, char acao) {
    ...
    if (!tSaida) {
        printf("Falha ao abrir o arquivo %s", dirSaida);
        exit(1);
    }

    fprintf(tSaida, "Estado do jogo apos o movimento '%c':\n", acao);
    ImprimeMapaNoDiretorio(jogo, tSaida);
    fprintf(tSaida, "Pontuacao: %d\n\n", RetornaPontuacao(jogo));
...
void RealizaJogo(tJogo jogo, char * diretorio) {
    ...
    while (scanf("%c%*c", &acao) == 1 && nMov != jogo.qtdJogadas) {
        ...
        // ImprimeMapa(jogo);
        // printf("Pontuacao: %d\n\n", RetornaPontuacao(jogo));
        SaidaArquivoSaida(jogo, diretorio, acao);

        jogo = AnalisaCoordenadasGameOver(jogo);
        if (!RetornaVidaPACMAN(jogo)) {
            RetornaGameOver(jogo, diretorio);
            exit(1);
        }
