...
tJogo AnalisaCoordenadas(tJogo jogo);
...
tJogo EfetuaJogadaPacman (tJogo jogo, char acao) {
    ...
    case 'w':
        if (x - 1 >= 0 && jogo.mapa[x-1][y] != PAREDE) {  
            if (jogo.mapa[x-1][y] == COMIDA) {
                stats.pacmanComeu++;
                jogo.stats = stats;
                jogo.mapa[x-1][y] = VAZIO;
            }
    ...
    case 'a':
        if (y - 1 >= 0 && jogo.mapa[x][y-1] != PAREDE) {
            if (jogo.mapa[x][y-1] == COMIDA) {
                stats.pacmanComeu++;
                jogo.stats = stats;
                jogo.mapa[x][y-1] = VAZIO;
            }
