...
tFantasmas IdentificaFantasmasPresentesECoord (tJogo jogo);
tEstatisticas ContabilizaEstatisticasIniciais (tJogo jogo); 
...
tJogo LeMapa (char *diretorio) {
    ...
    fclose (fMapa);
    jogo.stats = ContabilizaEstatisticasIniciais(jogo);
    return jogo;
}
...
tEstatisticas ContabilizaEstatisticasIniciais (tJogo jogo) {
    tEstatisticas stats;
    stats.nColisoesComParede = 0;
    stats.nMovimentoBaixo = 0;
    stats.nMovimentosCima = 0;
    stats.nMovimentoEsquerda = 0;
    stats.nMovimentoDireita = 0;
    stats.nMovimentosSemPontos = 0;
...
tFantasmas InicializaFantasmasSemGula(tFantasmas fant) {
    tFantasma1B fb;         tFantasma2P fp;         tFantasma3I fi;         tFantasma4C fc;
    fb.fantasmaGulao = 0 ;  fp.fantasmaGulao = 0;   fi.fantasmaGulao = 0;   fc.fantasmaGulao = 0;
    fant.fantB = fb;        fant.fantP = fp;        fant.fantI = fi;        fant.fantC = fc;
    return fant;
}
...
void RealizaJogo(tJogo jogo) {
    ...
    tEstatisticas stats;
    tFantasmas fants; fants = InicializaFantasmasSemGula(fants);
    jogo.dmov = 1;
    ...
    while (scanf("%c%*c", &aux) == 1 && stats.pacmanComeu != stats.qtdComidaJogo) {
        jogo = EfetuaJogadaPacman(jogo, aux, jogo.stats);

        fants = IdentificaFantasmasPresentesECoord(jogo);
        jogo.fantasmas = fants;
    ...
    case 'w':
        if (coord.x - 1 >= 0 && 
            jogo.mapa[coord.x-1][coord.y] != PAREDE) {  
                ...
                stats.nMovimentosSemPontos++;
                stats.nMovimentosCima++;
                jogo.stats = stats;
            ...
            if (fb.flag) {
                if (coord.y - jogo.dmov >= 0 || coord.y + jogo.dmov < jogo.nLinhas) {
                    if (jogo.mapa[coord.x][coord.y - jogo.dmov] != PAREDE) {
                        ...
                        if (fb.fantasmaGulao) {
                            jogo.mapa[fb.gulaB.x][fb.gulaB.y] = '*';
                            return jogo;
                        }
