...
tJogo EfetuaJogadaFantasmas (tJogo jogo, tFantasmas fant);
...
void RealizaJogo(tJogo jogo) {
    ...
    while (scanf("%c%*c", &aux) == 1 && jogo.stats.pacmanComeu != jogo.stats.qtdComidaJogo) {
        ...
        jogo = EfetuaJogadaFantasmas(jogo, fants);

        printf("\nEstado do jogo após o movimento '%c':\n", aux);
        ImprimeMapa(jogo);
        printf("\nPontuação: %d\n", jogo.stats.pacmanComeu);
        if (jogo.pacman.qtdVida == 0) {
            printf("GAME OVER!");
            exit(1);
        }
    }
...
tJogo EfetuaJogadaFantasmas (tJogo jogo, tFantasmas fant) {
    ...
    if (fb.presenteNoJogo) {
        ...
        if (coord.y - 1 >= 0) {
            if (fb.flag) {
                if (jogo.mapa[coord.x][coord.y - 1] != PAREDE) {
                    if (jogo.mapa[coord.x][coord.y - 1] == PACMAN) {
                        pac.qtdVida--;
                        jogo.pacman = pac;
                    }

                    jogo.mapa[coord.x][coord.y-1] = GHOST1;
                    jogo.mapa[coord.x][coord.y] = ' ';
                    return jogo;

                } else if (jogo.mapa[coord.x][coord.y - 1] == PAREDE) {
                    fb.flag = 0;
                    fant.fantB = fb;
                    jogo.fantasmas = fant;
                    jogo.mapa[coord.x][coord.y+1] = GHOST1;
